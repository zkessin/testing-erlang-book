...
how to build a type System. While Erlang is not a staticly typed language it is however a
strongly typed language, in that it will mostly not automaticly convert types for you
...
The way dialyzer works is treating the paramters and return values of your project
...
cup of tea. But you only have to build the PLT file once. You may have to redo it after masive changes
...
line use a command like this. This requires that ebin contail the beam files to
be examined that have been compiled with the *debug_info* flag. This can be done
with rebar by spedifying  _{erl_opts, [debug_info]}._ in the *rebar.config*
file.
