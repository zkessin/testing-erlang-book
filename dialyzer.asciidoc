...
work of Kostis Sagonas and his teams at Uppsala University (Sweden) and the
...
a plt file under a default name in your home directory. It is probably a
good idea to have a seperate PLT file for each project, as different 
projects may use different versions of dependencies and so on. 
...
code base. Normally dialyzer is run on the compiled *beam* file but it
...
really means is a function in an unknown module (if there is a known
module but no function of a given name or airity that will be reported
elsewhere) In this example, the system is showing three missing
functions. The first one is an actual problem as it represents a real
bug. In this case I had meant to call the function +emysql:execute/3+
but had left out the "y" in +emysql+ so of course this function does
not exist.
...
that was not included in the +plt+ file, so it should just b e added
to the plt file.
...
atoms or integers that represent themselves. So for example the
...
we could also make them more specific if we chose. 

However this does make the record definiton easier to understand as we
can  
