...
can provide Modified Decision/Condition Coverage. which is to say that instead of just
telling you if a given line of code was executed it can tell you if
each conditional on that line had all possible values tried. 
This can find cases where bugs can creap in despite test coverage due
to the fact that a possible condition is masked by other factors
...
If your functions are too long then, testing them becomes more difficult, in
this case it is probably a clue that your code needs to be refactored. Shorter
functions are easier to test and easier to reason about. 
