...
NOTE: Any valid atom is a legal function name in Erlang, so naming a test function something like
+'check that function does the right thing _test'+ is perfectly legal and will work just fine.
...
only run the tests of the actual project. You can make the output of rebar tests
more verbose with the +-v+ switch, which will do the same as the +verbose+ option
above. 

Rebar is my perfered method of running tests.
...
way that can prove illuminating. I have found that tests for the init
function of supervisors is often hard to test in a way that can prove
useful for the developer. These things should of course be subject to some
form of varification, but it does not have to be a unit test, it can be done in some other type of test.
