...
for this, mostly that when Erlang was being created that the people
...
The way dialyzer works is treating the parameters and return values of
...
constrains and repeats over several passes.
****
...
If your project, like mine, has a lot of dependences that you have
...
The Erlang type system can be very expresive, but it does not always
work the same way type systems in Object Oriented langauges such as
...
The basic types that are seen in Erlang will be familar to anyone who
...
type is equivilent to +integer()|float()+.
...
An array canb e indicated as one would expect with square brackets,
the type +[]+ indicates an empty array, while an aarray of integers
...
all possible successful results of this call, it will eather find the
item (presumably in a database) or return the atom +nothing+ to
indicate that there is no such item. The truly excpetional case of
...
However this does make the record definition easier to understand as
we can
